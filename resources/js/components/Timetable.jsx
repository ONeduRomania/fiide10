...
const TimetableComponent = (props) => {
    const { timetable, updateurl, deleteurl, createurl, subjects, teachers } =
        props;
    const [lessons, setLessons] = useState([]);
    const subjectsObj = useMemo(
        () =>
            JSON.parse(subjects || "[]").map((s) => ({
                id: s.id,
                text: s.name,
                value: s.id,
            })),
    ...
    const teachersObj = useMemo(
        () =>
            JSON.parse(teachers || "[]").map((s) => ({
                text: s.user.name,
                id: s.id,
                value: s.id,
            })),
    ...
    useEffect(() => {
        const lessons = JSON.parse(timetable || "[]").map((lesson) => {
            ...
            const currentWeekDay = isMonday(startDate)
                ? startOfISOWeek(today)
                : nextDay(today, dayOfWeek);
            return {
                event_id: lesson.id,
                ...
                color: "#00385b",
                start: set(new Date(currentWeekDay), {
                    hours: startDate.getHours(),
                    minutes: startDate.getMinutes(),
                }),
                end: set(new Date(currentWeekDay), {
    ...
    const updateLesson = async (newEvent, action) => {
        ...
        body.set("teacher_id", newEvent.teacher);
        ...
        try {
            if (action === "create") {
                ...
                return {
                    ...newEvent,
                    event_id: lesson.id,
                    title: subjectsObj.filter(
                        (s) => s.id === newEvent.subject
                    )[0].text,
                };
            } else {
            ...
            console.error(e);
            alert(
                "A apărut o eroare la modificarea orarului. Te rugăm să încerci din nou, iar dacă tot nu funcționează contactează-ne pentru a te putea ajuta."
            );
        }
    };
